...
$itemListId = $itemListId ?? null;
$itemListName = $itemListName ?? null;
$itemIndex = isset($itemIndex) ? (int) $itemIndex : null;
$selectItemPayload = \App\Support\Analytics::selectItemPayload($product, $itemListId, $itemListName, $itemIndex);
$selectItemPayloadJson = json_encode($selectItemPayload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_TAG | JSON_HEX_AMP);
...
<article class="product-card"
         ...
         data-analytics-select-item="{{ $selectItemPayloadJson }}"
    ...
    <div class="product-card__body product-card-body">
        ...
        <div class="product-card__footer">
            ...
            @if($wa)
            <a href="https://wa.me/{{ $wa }}?text={{ $waMsg }}" target="_blank" rel="noopener" class="product-card__wa" aria-label="Купить в WhatsApp" data-analytics-location="product_card" data-product-sku="{{ $product->sku }}" data-product-name="{{ $product->name }}">
